...
public class PupilGazeTracker : MonoBehaviour
{
    ...
    public void StartCalibration() //calibrate using 17 points and 120 samples for each target
    {
        StartCalibration(new Vector2[] {new Vector2 (0.5f, 0.5f),
            new Vector2 (0.25f, 0.25f), new Vector2 (0.25f, 0.5f),
            new Vector2 (0.25f, 0.75f), new Vector2 (0.5f, 0.75f), new Vector2 (0.75f, 0.75f),
            new Vector2 (0.75f, 0.5f), new Vector2 (0.75f, 0.25f), new Vector2 (0.5f, 0.25f),
            new Vector2(0.35f,0.35f), new Vector2(0.35f,0.5f), new Vector2(0.35f,0.65f), new Vector2(0.5f,0.65f),
            new Vector2(0.65f,0.65f),new Vector2(0.65f,0.5f),new Vector2(0.65f,0.35f),new Vector2(0.5f,0.35f)
    ...
    void OnGUI()//GUI with eye pos, capture rate, time left, VP Number entry field -> only seen by experimenter, not in VR
    {
        ...
        PortIn = GUI.TextField(new Rect(0, 110, 200, 20), PortIn, 25);
        if (GUI.Button(new Rect(0, 90, 200, 20), "Set VP Number"))
        {
            VPNum = VPNumIn;
        }
        if (GUI.Button(new Rect(0, 130, 200, 20), "Set Port"))
        {
            ServicePort = Int32.Parse(PortIn);
            _serviceThread = new Thread(NetMQClient);
            _serviceThread.Start();
        }
    }
